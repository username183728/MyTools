...
Kumpulan tools developer untuk Android (Kivy + KivyMD 1.2.0): File Manager, Text Editor,
runner Python, Terminal, ZIP/UNZIP, HTTP Server, Installer pip, APK Builder (Buildozer),
Wi-Fi Info, dan alat cepat (JSON, Hash, Regex, DNS, Ping, dst).

Jalankan di PC / Termux:
    pip install kivy kivymd==1.2.0 pillow qrcode psutil certifi
    python main.py

Build APK: lihat buildozer.spec dan .github/workflows/build-apk.yml.
Catatan penting untuk buildozer.spec APLIKASI INI:
  - android.permissions harus memuat MANAGE_EXTERNAL_STORAGE (File Manager di Android 11+),
    REQUEST_INSTALL_PACKAGES (installer APK) dan izin Wi-Fi/lokasi (tool Wi-Fi Info).
  - requirements harus memuat certifi (HTTPS di Android) dan qrcode/pypng (QR Generator).
  - android.enable_androidx, android.res_xml dan android.extra_manifest_application_arguments
    dipakai oleh FileProvider (provider_paths.xml + extra_manifest_application_arguments.xml).

Batasan yang perlu diketahui:
  - APK Builder butuh `buildozer` + Android SDK/NDK di PATH. Di HP biasa itu TIDAK ada, jadi
    build hanya jalan di PC / Termux dengan toolchain lengkap (atau pakai GitHub Actions).
  - Runner Python memakai python3 di PATH bila ada; kalau tidak, script dijalankan di
    dalam proses MyTools sendiri (Stop = interupsi paksa, bukan kill proses).
  - HTTP Server mode LAN tidak memakai autentikasi.
"""
...
# HTTPS di Android: OpenSSL bawaan p4a tidak membaca CA sistem -> pakai bundle certifi.
try:
    import certifi
...
def android_all_files_granted():
    """True bila izin 'Akses semua file' aktif (Android 11+) atau tidak diperlukan."""
    if platform != "android":
        return True
    try:
        ...
        return True   # tidak bisa dicek -> jangan mengganggu pengguna


def tail_text(text, limit):
    """Potong bagian depan teks sampai <= limit, dimulai dari awal baris
    (supaya entitas markup hasil escape tidak terpotong di tengah)."""
    if len(text) <= limit:
        return text
    text = text[-limit:]
    nl = text.find("\n")
...
class BatchedSink:
    """Kumpulkan output dari thread background lalu kirim ke UI paling sering
    tiap `interval` detik (mencegah ribuan event Clock + render ulang Label)."""

    def __init__(self, callback, interval=0.12):
    ...
    def push(self, text):
        ...
        Clock.schedule_once(self.drain, self.interval)

    def drain(self, *_):
        """Harus dipanggil di thread UI."""
        with self._lock:
            data = "".join(self._buf)
            self._buf = []
            self._scheduled = False
...
def popen_extra():
    """Argumen Popen: stdin dialihkan ke DEVNULL (cegah perintah menggantung menunggu
    input) dan proses dibuat sebagai grup baru supaya bisa dihentikan beserta anaknya."""
...
def kill_process_tree(proc):
    """Hentikan proses beserta proses anaknya (bukan cuma shell-nya)."""
    if proc is None:
        return
    try:
        if os.name == "posix":
...
# =====================================================================
FM_MAX_ITEMS = 500
SCAN_INFO = {"total": 0}   # jumlah item asli pada scan_dir() terakhir (untuk info terpotong)


def _count_entries(path, cap=200):
    n = 0
    try:
...
def qtfn_base64(a, b, mode):
    try:
        if mode == "Decode":
            s = "".join(a.split())
            padded = s + "=" * (-len(s) % 4)
            if "-" in s or "_" in s:   # varian URL-safe
                raw = base64.urlsafe_b64decode(padded.encode())
            else:
                raw = base64.b64decode(padded.encode(), validate=True)
...
def qtfn_regex(a, b, mode):
    ...
    text = b or ""
    if len(a) > 500 or len(text) > 20000:
        return "❌ Pola maks. 500 karakter dan teks maks. 20.000 karakter (cegah UI membeku)"
    if mode == "Cocok?":
        m = pat.search(text)
        if not m:
            return "❌ Tidak cocok"
...
def qtfn_color(a, b, mode):
    try:
        if mode == "RGB → Hex":
            ...
            if not all(0 <= v <= 255 for v in (r, g, bb)):
                return "❌ Nilai RGB harus antara 0 dan 255"
...
# alat cepat yang menunggu jaringan -> dijalankan di thread background
QT_THREADED = {"dnslookup", "reversedns", "portcheck", "publicip", "ping"}
            ...
            MDBoxLayout:
                ...
                TapBox:
                    ...
                    T1:
                        ...
                        halign: "center"
                    T4:
                        text: "Buildozer dijalankan sungguhan. Log di bawah diperbarui real-time."
                        halign: "center"
                MDBoxLayout:
                    size_hint_y: None
                    height: dp(24)
                ...
                MDBoxLayout:
                    ...
                    MDBoxLayout:
                        ...
                        PrimaryBtn:
                            ...
                            icon: "stop"
                            outlined: True
                            disabled: not app.python_running
                            on_release: app.stop_python()
                    Sec:
                        text: "Output"
                    MDBoxLayout:
                        adaptive_height: True
    ...
    MDBoxLayout:
        ...
        MDBoxLayout:
            ...
            MDIconButton:
                ...
                theme_icon_color: "Custom"
                icon_color: WHITE
                disabled: not app.terminal_running
                on_release: app.stop_terminal()
...
<EditorScreen@Page>:
    MDBoxLayout:
        ...
        TopBar:
            title: "Text Editor"
            icons: ["undo", "redo", "content-save-outline"]
        PathChip:
            id: ed_chip
            icon: "file-document-outline"
            path: "Tanpa judul"
        MDBoxLayout:
            ...
            Boxed:
                ...
                TextInput:
                    ...
                    font_name: "data/fonts/RobotoMono-Regular.ttf"
                    font_size: sp(13)
                    hint_text: "Ketik kode atau teks di sini..."
                    multiline: True
                    background_normal: ""
                    background_active: ""
                    background_color: CLEAR
        ...
        BottomBar:
            ...
            BarItem:
                icon: "select-all"
                text: "Pilih"
                on_release: app.editor_select_all()
...
class MyToolsApp(MDApp):
    ...
    # ---- teks layar Python / Terminal / Build (semua sudah di-escape untuk markup) ----
    CODE_TEXT = StringProperty("")
    CODE_LINES = StringProperty("")
    OUTPUT_TEXT = StringProperty("Belum ada output. Tekan Jalankan untuk menjalankan file di atas.")
    LOG_TEXT = StringProperty("Belum ada build. Pilih project lalu tekan Mulai Build.")
    ...
    python_running = BooleanProperty(False)
    terminal_running = BooleanProperty(False)
    ...
    def build(self):
        ...
        self._code_proc = None
        self._code_thread = None
        self._code_thread_id = None
        self._qt_busy = False
        self._all_files_prompted = False
        self.building_active = False
        self._build_proc = None
        self._term_proc = None
        self._term_cwd = storage_path()
    ...
    def on_start(self):
        ...
        self.TERM_TEXT = escape_markup(self._terminal_prompt())
        self._refresh_code_preview()
        self.root.bind(current=lambda inst, val: self._on_screen_changed(val))
    ...
    def _after_permissions(self):
        ...
        self._ensure_all_files_access()

    def on_resume(self):
        # kembali dari Pengaturan (mis. setelah memberi "Akses semua file") -> segarkan daftar
        if platform == "android":
            self._reload_all_lists()
        return True

    def _on_screen_changed(self, name):
        if name == "python":
            self._refresh_code_preview()

    def _ensure_all_files_access(self):
        """Android 11+: tanpa 'Akses semua file' File Manager/Editor/ZIP tidak bisa
        menjangkau penyimpanan bersama. Tawarkan sekali saja per sesi."""
        if android_all_files_granted() or self._all_files_prompted:
            return
        self._all_files_prompted = True
        ConfirmDialog(
            title="Izinkan akses semua file",
            message=("File Manager, Text Editor dan ZIP butuh izin \"Akses semua file\" "
                     "di Android 11+. Ketuk Buka Pengaturan lalu aktifkan MyTools."),
            ok_text="Buka Pengaturan",
            callback=self._open_all_files_settings,
        ).open()

    def _open_all_files_settings(self):
        activity = self._get_android_activity()
        if activity is None:
            self.show_toast("Tidak bisa membuka Pengaturan")
            return
        try:
            from jnius import autoclass
            ...
            self.show_toast(f"Gagal membuka Pengaturan: {str(e)[:60]}")

    def _refresh_code_preview(self):
        """Tampilkan awal file Python yang akan dijalankan (bukan kode contoh)."""
        path = self._python_target_path()
        if not path or not os.path.isfile(path):
            self.CODE_LINES = ""
            self.CODE_TEXT = escape_markup(
                "Belum ada file Python.\nBuka file .py di Text Editor\n"
                "atau pilih project yang punya main.py.")
            return
        shown, total = [], 0
        try:
            with open(path, "r", encoding="utf-8", errors="replace") as f:
                for ln in f:
                    total += 1
                    if len(shown) < 9:
                        shown.append(ln.rstrip("\r\n")[:60])
        except OSError as e:
            self.CODE_LINES = ""
            self.CODE_TEXT = escape_markup(f"Tidak bisa membaca file: {e}")
            return
        numbers = [str(i + 1) for i in range(len(shown))]
        if total > len(shown):
            shown.append(f"... (+{total - len(shown)} baris lagi)")
            numbers.append("")
        self.CODE_LINES = "\n".join(numbers)
        self.CODE_TEXT = "\n".join(escape_markup(x) for x in shown)
    ...
    def _reload_list(self, screen_name):
        ...
        if d:
            try:
                ...
                if SCAN_INFO["total"] > FM_MAX_ITEMS and screen_name == self.root.current:
                    self.show_toast(f"Menampilkan {FM_MAX_ITEMS} dari {SCAN_INFO['total']} item")
            except Exception as e:
                cfg["error"] = str(e)[:80]
    ...
    # ------------------------------------------------------------ izin & util
    def _request_permissions(self, on_result=None, wifi=False):
        """Minta izin runtime. wifi=False: storage (Android <=10); wifi=True: lokasi /
        NEARBY_WIFI_DEVICES untuk membaca SSID. on_result dipanggil di thread UI."""
        def finish():
            if on_result:
                on_result()
        ...
        try:
            ...
            names = (("ACCESS_FINE_LOCATION", "ACCESS_COARSE_LOCATION", "NEARBY_WIFI_DEVICES")
                     if wifi else ("READ_EXTERNAL_STORAGE", "WRITE_EXTERNAL_STORAGE"))
            ...
            request_permissions(missing, _on_answered)
        except Exception:
            finish()
    ...
    def editor_select_all(self):
        ed = self._ed().ed_text
        ed.select_all()
        self.show_toast("Semua teks dipilih")
    ...
    def http_start(self):
        ...
        if host == "0.0.0.0":
            ConfirmDialog(
                title="Buka ke jaringan Wi-Fi?",
                message=("Semua perangkat di jaringan yang sama bisa membaca isi folder ini "
                         "TANPA password:\n" + short_text(self.http_dir, 40)),
                ok_text="Lanjutkan",
                callback=lambda: self._http_launch(host, port),
            ).open()
            return
        self._http_launch(host, port)

    def _http_launch(self, host, port):
        if self.http_server is not None:
            return
        try:
    ...
    def on_top_icon(self, name):
        ...
        if self.root.current == "editor":
            if name in ("content-save-edit-outline", "content-save-outline"):
                self.editor_save()
            elif name == "undo":
                self.editor_undo()
            elif name == "redo":
        ...
        elif name == "dots-vertical" and self.root.current in self._lists:
            self.on_options()
    ...
    def on_start_build(self):
        ...
        if not ok:
            self._log_set(f"{datetime.now().strftime('%H:%M')}  ERROR: {message}")
            self.show_toast(message)
            self.goto("building")
            return
        ...
        if not buildozer_bin:
            self._log_add(
                "\nBuildozer tidak ditemukan di perangkat ini.\n"
                "Build APK butuh Buildozer + Android SDK/NDK + Java/Gradle, yang umumnya "
                "tidak ada di HP biasa. Pilihan: jalankan MyTools di PC / Termux dengan "
                "toolchain lengkap, atau build lewat GitHub Actions "
                "(.github/workflows/build-apk.yml) lalu install APK hasilnya."
            )
    ...
    def _log_set(self, text):
        self.LOG_TEXT = tail_text(escape_markup(text), 40000)

    def _log_add(self, text):
        self.LOG_TEXT = tail_text(self.LOG_TEXT + escape_markup(text), 40000)

    def _build_worker(self, args, cwd):
        """Jalan di background thread: eksekusi buildozer sungguhan & streaming log (per batch)."""
        rc = -1
        sink = BatchedSink(self._build_push, interval=0.25)
        try:
            env = os.environ.copy()
            proc = subprocess.Popen(args, cwd=cwd, stdout=subprocess.PIPE, stderr=subprocess.STDOUT,
                                     text=True, errors="replace", bufsize=1, env=env, **popen_extra())
            self._build_proc = proc
            for line in proc.stdout:
                sink.push(line)
            rc = proc.wait()
        except FileNotFoundError:
            sink.push("Error: perintah buildozer tidak ditemukan\n")
        except Exception as e:
            sink.push(f"Error: {e}\n")
        finally:
            self._build_proc = None
        Clock.schedule_once(lambda dt: (sink.drain(), self._build_done(rc, cwd)))

    def _build_push(self, chunk):
        """Terima satu batch baris log; tambahkan ke log & tebak langkah yang sedang berjalan."""
        chunk = chunk.rstrip("\n")
        if not chunk:
            return
        self._log_add("\n" + chunk)
    ...
    def build_cancel(self):
        ...
        if self._build_proc:
            kill_process_tree(self._build_proc)
    ...
    def on_share_apk(self):
        """Bagikan APK hasil build melalui Android share sheet."""
        path = self._built_apk_path
        if not path or not os.path.isfile(path):
            self.show_toast("APK hasil build tidak ditemukan")
            return
        if platform != "android":
            self.copy_text(path, "Path APK disalin")
            return
        try:
            from jnius import autoclass
            Intent = autoclass("android.content.Intent")
            ...
            activity = self._get_android_activity()
            context = activity.getApplicationContext()
            uri = Provider.getUriForFile(context, context.getPackageName() + ".fileprovider", JFile(path))
            send = Intent(Intent.ACTION_SEND)
            send.setType("application/vnd.android.package-archive")
            send.putExtra(Intent.EXTRA_STREAM, uri)
            send.addFlags(Intent.FLAG_GRANT_READ_URI_PERMISSION)
            activity.startActivity(Intent.createChooser(send, "Bagikan APK"))
        except Exception as e:
            self.show_toast(f"Gagal berbagi: {str(e)[:80]}")

    def on_open_location(self):
        """Buka folder output APK; jika file manager tidak mendukung, salin lokasinya."""
        path = self._built_apk_path
        if not path or not os.path.isfile(path):
            self.show_toast("Lokasi APK tidak ditemukan")
            return
        folder = os.path.dirname(path)
        if platform != "android":
            self.copy_text(folder, "Lokasi folder disalin")
            return
        try:
            from jnius import autoclass
            Intent = autoclass("android.content.Intent")
            ...
            activity = self._get_android_activity()
            intent = Intent(Intent.ACTION_VIEW)
            intent.setDataAndType(Uri.parse("file://" + folder), "resource/folder")
            intent.addFlags(Intent.FLAG_ACTIVITY_NEW_TASK)
            activity.startActivity(intent)
        except Exception:
            self.copy_text(folder, "Lokasi APK disalin")
    ...
    def _python_target_path(self):
        """Ambil file Python yang sedang dibuka; fallback ke main.py project."""
        if self.editor_path and self.editor_path.lower().endswith(".py"):
            return self.editor_path
        if self._apk_project_dir:
            candidate = os.path.join(self._apk_project_dir, "main.py")
            if os.path.isfile(candidate):
                return candidate
        return None

    def on_run_code(self):
        """Jalankan file Python sungguhan dan tampilkan stdout/stderr."""
        if self.python_running:
            return
        path = self._python_target_path()
        if not path:
            self.show_toast("Buka file .py atau pilih project yang punya main.py")
            return
        self.python_running = True
        self._running_code = True
        self._refresh_code_preview()
        self.OUTPUT_TEXT = escape_markup(f"Menjalankan: {os.path.basename(path)}") + "\n"
        sink = BatchedSink(self._code_push)

        def worker():
            rc = 1   # selalu terdefinisi, apa pun yang terjadi di bawah
            try:
                py = shutil.which("python3") or shutil.which("python")
                if py:
                    proc = subprocess.Popen(
                        [py, "-u", path], cwd=os.path.dirname(path),
                        stdout=subprocess.PIPE, stderr=subprocess.STDOUT,
                        text=True, errors="replace", bufsize=1, **popen_extra(),
                    )
                    self._code_proc = proc
                    for line in proc.stdout:
                        sink.push(line)
                    rc = proc.wait()
                else:
                    # Tidak ada python di PATH (umum di HP): jalankan di dalam proses ini.
                    rc = self._run_inprocess(path, sink)
            except BaseException as e:   # termasuk SystemExit / KeyboardInterrupt dari script
                rc = 1
                sink.push(f"{type(e).__name__}: {e}\n")
            finally:
                self._code_proc = None
                self._code_thread_id = None
                Clock.schedule_once(lambda dt, r=rc, p=path: (sink.drain(), self._code_done(r, p)))

        self._code_thread = threading.Thread(target=worker, daemon=True)
        self._code_thread.start()

    def _run_inprocess(self, path, sink):
        """Eksekusi script di thread ini dengan stdout/stderr dialihkan ke `sink`.
        Mengembalikan exit code. sys.exit() dan error di script ditangani di sini."""
        import contextlib
        ...
        self._code_thread_id = threading.get_ident()
        ...
        try:
            ...
            with contextlib.redirect_stdout(_Out()), contextlib.redirect_stderr(_Out()):
                try:
                    ...
                    # lewati frame internal MyTools (baris exec) supaya traceback hanya milik script
                    traceback.print_exception(et, ev, tb.tb_next if tb is not None else None)
                    rc = 1
        except OSError as e:
            sink.push(f"Tidak bisa membuka file: {e}\n")
            rc = 1
        finally:
            sys.argv = old_argv
            sys.path[:] = old_path
    ...
    def _code_push(self, chunk):
        self.OUTPUT_TEXT = tail_text(self.OUTPUT_TEXT + escape_markup(chunk), 12000)

    def _code_done(self, rc, path):
        self.python_running = False
        self._running_code = False
        status = "selesai" if rc == 0 else f"gagal (kode {rc})"
        self.OUTPUT_TEXT += "\n" + escape_markup(f"--- {os.path.basename(path)} {status} ---")
        self.show_toast("Python selesai" if rc == 0 else "Python error")

    def stop_python(self):
        if self._code_proc:
            kill_process_tree(self._code_proc)
            self.show_toast("Proses Python dihentikan")
            return
        tid = self._code_thread_id
        if tid is None:
            return
        try:
            import ctypes
            ...
            if n > 1:   # lebih dari satu thread terkena -> batalkan
                ctypes.pythonapi.PyThreadState_SetAsyncExc(ctypes.c_ulong(tid), None)
            self.show_toast("Script diinterupsi (berhenti di baris Python berikutnya)")
        except Exception as e:
            self.show_toast(f"Gagal menghentikan: {str(e)[:60]}")

    # Terminal
    def _terminal_prompt(self):
        cwd = self._term_cwd or storage_path()
        return f"$ {cwd} > "

    def on_send_command(self, cmd):
        """Jalankan perintah shell sungguhan dalam background dan streaming output."""
        cmd = cmd.strip()
        if not cmd or self.terminal_running:
            return
        ...
        self._term_cwd = self._term_cwd if os.path.isdir(self._term_cwd) else storage_path()
        self._term_append(f"{self._terminal_prompt()}{cmd}\n")

        # Built-in shell navigation harus mengubah cwd MyTools sendiri, bukan cwd
        # proses anak yang langsung berakhir setelah perintah selesai.
        parts = cmd.split(maxsplit=1)
        if parts and parts[0] == "cd":
            target = parts[1].strip() if len(parts) > 1 else storage_path()
            if target == "~":
                target = storage_path()
            elif not os.path.isabs(target):
                target = os.path.join(self._term_cwd, target)
            target = os.path.abspath(os.path.expanduser(target))
            if os.path.isdir(target):
                self._term_cwd = target
                self._term_append(f"{self._term_cwd}\n{self._terminal_prompt()}")
            else:
                self._term_append(f"cd: folder tidak ditemukan: {target}\n{self._terminal_prompt()}")
            return

        self.terminal_running = True
        sink = BatchedSink(self._term_append)

        def worker():
            rc = 1
            try:
                env = os.environ.copy()
                shell = "/system/bin/sh" if platform == "android" and os.path.exists("/system/bin/sh") else None
                proc = subprocess.Popen(
                    cmd, cwd=self._term_cwd, shell=True, executable=shell,
                    stdout=subprocess.PIPE, stderr=subprocess.STDOUT,
                    text=True, errors="replace", bufsize=1, env=env, **popen_extra(),
                )
                self._term_proc = proc
                for line in proc.stdout:
                    sink.push(line)
                rc = proc.wait()
            except Exception as e:
                ...
                self._term_proc = None
                Clock.schedule_once(lambda dt, r=rc: (sink.drain(), self._term_done(r)))

        threading.Thread(target=worker, daemon=True).start()

    def _term_append(self, text):
        """Tambah teks polos ke layar terminal (di-escape supaya '[' / ']' tampil apa adanya)."""
        self.TERM_TEXT = tail_text(self.TERM_TEXT + escape_markup(text), 16000)

    def _term_push(self, line):
        self._term_append(line)

    def _term_done(self, rc):
        self.terminal_running = False
        self._term_append(f"\n[exit {rc}]\n{self._terminal_prompt()}")

    def stop_terminal(self):
        if self._term_proc:
            kill_process_tree(self._term_proc)
            self.show_toast("Perintah dihentikan")
    ...
    def on_open_tool(self, name):
        if name == "system":
            ...
            self.refresh_wifi_info()
            self.goto("wifiinfo")
            # izin lokasi hanya diminta saat tool ini dibuka; setelah dijawab, data dimuat ulang
            self._request_permissions(on_result=self.refresh_wifi_info, wifi=True)
        elif name == "checksum":
            self.goto("checksum")
        elif name == "logviewer":
            self.refresh_log_viewer()
    ...
    def qt_run(self):
        ...
        if (spec.get("hint1") and not spec.get("generate_only")
            ...
            return
        if self._qt_key in QT_THREADED:
            # DNS / Port / Ping / IP publik bisa menunggu beberapa detik -> jangan blok UI
            if self._qt_busy:
                self.show_toast("Masih memproses...")
                return
            self._qt_busy = True
            scr.ids.qt_output.text = "Memproses..."
            func, mode, key = spec["func"], self._qt_mode, self._qt_key

            def worker():
                try:
                    res = str(func(a, b, mode))
                except Exception as e:
                    res = f"❌ Terjadi kesalahan: {e}"
                Clock.schedule_once(lambda dt: self._qt_finish(key, res))
    ...
    def _qt_finish(self, key, result):
        self._qt_busy = False
        if self._qt_key != key:   # pengguna sudah pindah ke alat lain
            return
        scr = self.root.get_screen("quicktool")
        if "qt_output" in scr.ids:
            scr.ids.qt_output.text = result
    ...
    def refresh_log_viewer(self):
        ...
        parts = ["[b]Build[/b]", self.LOG_TEXT]   # LOG_TEXT sudah ter-escape
